// script.js
let regFirstName, regLastName, regEmail, regPassword, regConfirmPassword, regPhone, regDob, regCountry, regCity;
let firstNameError, lastNameError, emailError, passwordError, confirmPasswordError, phoneError, dobError, sexError, countryError, cityError;
let regForm, registerMessage, loginUsername, loginPassword, loginRemember, loginForm, loginMessage, loginUsernameError, loginPasswordError;
let tabBtns, forms;

const validateName = (name) => {
    if (!name || name.trim().length === 0) return { isValid: false, error: "Поле обов'язкове" };
    const trimmed = name.trim();
    if (trimmed.length < 3 || trimmed.length > 15) return { isValid: false, error: "Має бути від 3 до 15 символів" };
    const regex = /^[A-Za-zА-Яа-яЄєІіЇїҐґ'\- ]+$/;
    if (!regex.test(trimmed)) return { isValid: false, error: "Тільки літери, пробіли, дефіс, апостроф" };
...
const validateEmail = (email) => {
    ...
    const re = /^[^\s@]+@([^\s@]+\.)+[^\s@]+$/;
    if (!re.test(email)) return { isValid: false, error: "Введіть коректний email (user@domain.com)" };
    return { isValid: true, error: "" };
};

const validatePassword = (password) => {
    if (!password) return { isValid: false, error: "Пароль обов'язковий" };
    if (password.length < 6) return { isValid: false, error: "Пароль має містити мінімум 6 символів" };
    return { isValid: true, error: "" };
};

const validateConfirmPassword = (password, confirm) => {
    if (confirm === undefined || confirm === "") return { isValid: false, error: "Підтвердіть пароль" };
    if (password !== confirm) return { isValid: false, error: "Паролі не співпадають" };
    return { isValid: true, error: "" };
};
...
const validatePhone = (phone) => {
    ...
    const regex = /^\+380\d{9}$/;
    if (!regex.test(cleaned)) return { isValid: false, error: "Формат: +380XXXXXXXXX (9 цифр)" };
    return { isValid: true, error: "" };
};

const validateAge = (dateString) => {
    if (!dateString) return { isValid: false, error: "Дата народження обов'язкова" };
    const birthDate = new Date(dateString);
    const today = new Date();
    if (birthDate > today) return { isValid: false, error: "Дата не може бути в майбутньому" };
    let age = today.getFullYear() - birthDate.getFullYear();
    const monthDiff = today.getMonth() - birthDate.getMonth();
    if (monthDiff < 0 || (monthDiff === 0 && today.getDate() < birthDate.getDate())) age--;
    if (age < 12) return { isValid: false, error: "Вам має бути не менше 12 років для реєстрації" };
    return { isValid: true, error: "" };
};

const validateSex = (sexValue) => {
    if (!sexValue) return { isValid: false, error: "Оберіть стать" };
    return { isValid: true, error: "" };
};
...
const validateUsername = (username) => {
    if (!username || username.trim() === "") return { isValid: false, error: "Введіть логін" };
    return { isValid: true, error: "" };
};
...
function updateCityOptions(countryValue) {
    if (!countryValue) {
        ...
        regCity.innerHTML = '<option value="" selected>Спочатку оберіть країну</option>';
        return;
    }
    const cities = citiesMap[countryValue] || [];
    regCity.disabled = false;
    regCity.innerHTML = '<option value="" disabled selected>Оберіть місто</option>';
    cities.forEach(city => {
...
function validateRegistrationForm() {
    ...
    if (!sexValid.isValid) {
        sexError.textContent = sexValid.error;
    } else {
    ...
    return nameValid.isValid && lastNameValid.isValid && emailValid.isValid && passwordValid.isValid &&
           confirmValid.isValid && phoneValid.isValid && ageValid.isValid && sexValid.isValid &&
           countryValid.isValid && cityValid.isValid;
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    regForm.addEventListener('submit', (e) => {
        ...
        if (validateRegistrationForm()) {
            ...
            setTimeout(() => {
                if(registerMessage.textContent.includes('виправте')) registerMessage.textContent = '';
            }, 3000);
        }
    });
    ...
    loginForm.addEventListener('submit', (e) => {
        ...
        if (validateLoginForm()) {
            ...
            setTimeout(() => {
                if(loginMessage.textContent.includes('Невірний')) loginMessage.textContent = '';
            }, 3000);
        }
    });
    ...
    addBlurValidation('regFirstName', (v) => validateName(v), 'firstNameError', (v) => v.trim());
    addBlurValidation('regLastName', (v) => validateName(v), 'lastNameError', (v) => v.trim());
    addBlurValidation('regEmail', (v) => validateEmail(v), 'emailError', (v) => v.trim());
    addBlurValidation('regPassword', validatePassword, 'passwordError');
    addBlurValidation('regConfirmPassword', (v) => validateConfirmPassword(regPassword.value, v), 'confirmPasswordError');
    addBlurValidation('regPhone', (v) => validatePhone(v), 'phoneError', (v) => v.trim());
    ...
    addBlurValidation('loginUsername', (v) => validateUsername(v), 'loginUsernameError', (v) => v.trim());
